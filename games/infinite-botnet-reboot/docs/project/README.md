...
- Reboot tracker: `../tracker/REFONTE_TRACKER.md`.
- Edition map by gameplay system: `WHERE_TO_EDIT.md`.
...
src/
  ...
  styles/
    ...
    20-controls.css     # Buttons, meters, form controls, metrics
    30-dashboard.css    # Dashboard widgets (resources, upgrades, console)
    40-overlays.css     # Overlay base + settings panels
    41-overlays-lore.css # Lore cinematic + transition bridge
    42-overlays-guide.css # Guide overlay and spotlight mask
    50-motion.css       # Keyframes and reduced-motion fallback
    60-responsive.css   # Media queries and layout breakpoints
  content/
    fr/
  ...
  worker/
    ...
    domain/
      phases/           # Phase ladder definitions + progression resolver
      economy/          # Economy split: helpers, deriveRates, tick, commands
      war.ts            # Heat/war mechanics and tactical commands
      matrix.ts         # Matrix loop (bypass/inject/stabilize/collapse)
      narrative/        # Narrative split: templates, rewards, generation, commands
      upgrades/         # Upgrade chains, prerequisites, effects, purchase flow
  sim/
