...
function pickTemplate(bucket: NarrativeBucket): FrNarrativeTemplate {
  const candidates = FR_NARRATIVE_CATALOG[bucket];
...
function computeNextIntervalMs(state: EngineState): number {
  ...
  const base = state.rates.messageIntervalBaseMs;
